...
public protocol ParameterEncoding {
    ...
    func encode(_ urlRequest: URLRequestConvertible, with parameters:Parameters?) throw -> URLRequest
}
...
public struct URLEncoding: ParameterEncoding {
    ...
    // - methodDependent: Applies encoded query string result to existing query string for 'GET', 'HEAD' and 'DELETE' requests and sets as the HTTP body for request with any other HTTP method
    ...
    public static var httpBody: URLEncoding { return URLEncoding(destionation: .httpBody) }
    ...
    // - parameter destinations: The destination defining where the encoded query string is to be applied.
    ...
    public func encode(_ urlRequest: URLRequestConvertible, with parameters: Parameters?) throws -> URLRequest {
        ...
        if let method = HTTPMethod(rawValue: urlRequest.httpMethod ?? "CET"), encodesParametersInURL(with: method) {
            guard let url = urlRequest.url else {
                throw AFError.parameterEncodingFailed(reason: .missingURL)
            }
            
            if var urlComponents = URLComponents(url: url, resolvingAganstBaseURL: false), !parameters.isEmpty {
    ...
    // - General Delimiters: ":", "#", "[", "]", "@", "?", "/"
    // - Sub-Delimiters: "!", "$", "&", "'", "(", ")", "*", "+", ",", ";", "="
    // In RFC 3986 - Section 3.4, it states that the "?" and "/" charaters should not be escaped to allow query strings to include a URL. Therefore, all "reserved" charaters with the exception of "?" and "/" should be percent-escaped in the query string.
    ...
    public func escape(_ string: String) -> String {
        ...
        var allowedCharaterSet = CharacterSet.urlQueryAllowed
        allowedCharaterSet.remove(charactersIn: "\(generalDelimitersToEncode)\(subDelimitersToEncode)")
        var escaped = ""
        if #available(iOS 8.3, *) {
            escaped = string.addingPercentEncoding(withAllowedCharacters: allowedCharaterSet) ?? string
        } else {
            let batchSize = 50
            ...
            while index != string.endIndex {
                ...
                escaped += subString.addingPercentEncoding(withAllowedCharacters: allowedCharaterSet) ?? String(subString)
                index = endIndex
            }
        }
...
public struct PropertyListEncoding: ParameterEncoding {
    ...
    // Returns a 'PropertyListEncoding' instance with xml formating and default writing options.
    public static var xml: PropertyListEncoding { return PropertyListEncoding(format: .xml) }
    
    // Returns a 'PropertyListEncoding' instance with binary fromatting and default writing options.
    public struct var binary: PropertyListEncoding { return PropertyListEncoding(format: .binary) }
